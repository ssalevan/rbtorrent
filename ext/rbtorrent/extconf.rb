...
def runSwig(swig, args)
  system "#{swig} #{args} -autorename -c++ -ruby -o rbtorrent.cpp rbtorrent.i"
end

idir, ldir = dir_config("libtorrent", "/usr/local/include", "/usr/local/lib")
dir_config("boost", "/usr/local/include", "/usr/local/lib")

have_library "boost"
have_library "libtorrent-rasterbar"
pkg_config "boost"
...
if ! File.exists?("rbtorrent.cpp") || dependenciesModifiedSince?(File.mtime("rbtorrent.cpp"))
...
$distcleanfiles << "rbtorrent.cpp"

create_makefile("rbtorrent")
